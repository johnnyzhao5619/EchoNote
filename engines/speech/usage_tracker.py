...
class UsageTracker:
    ...
    def record_usage(
        self, engine: str, duration_seconds: float, timestamp: Optional[datetime] = None
    ) -> str:
        """
        记录 API 使用量
        ...
        try:
            ...
            logger.info(
                f"Recorded usage: engine={engine}, duration={duration_seconds}s, cost=${cost}"
            )
            return record_id
    ...
    def get_monthly_usage(
        self, engine: Optional[str] = None, year: Optional[int] = None, month: Optional[int] = None
    ) -> Dict:
        """
        获取月度使用统计
    ...
    def get_usage_history(
        self, engine: Optional[str] = None, days: int = 30, limit: int = 100
    ) -> list:
        """
        获取使用历史记录
