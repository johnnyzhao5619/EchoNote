...
class OutlookCalendarAdapter(OAuthCalendarAdapter):
    ...
    def fetch_events(
        ...
        try:
            ...
            if e.response.status_code == 410:
                ...
                self.logger.warning("Delta link expired, performing full sync")
    ...
    def delete_event(self, event: CalendarEvent, external_id: str) -> None:
        ...
        try:
            ...
            if err.response.status_code == 404:
                self.logger.warning("Outlook event %s already removed", external_id)
                return
            self.logger.error("HTTP error deleting Outlook event %s: %s", external_id, err)
            raise
        except Exception as e:
            self.logger.error("Failed to delete Outlook event %s: %s", external_id, e)
            raise

    def _convert_outlook_event(self, outlook_event: dict) -> Optional[dict]:
        """
        ...
        try:
            ...
            if removed_payload is not None:
                ...
                if event_id and (not outlook_event.get("start") or not outlook_event.get("end")):
            ...
            def _convert_datetime(
                ...
                *,
                ...
                identifier = _normalise_identifier(payload.get("timeZone") or fallback_identifier)
                ...
                try:
                    parsed_value = datetime.fromisoformat(_prepare_datetime_text(value))
                except ValueError:
                    self.logger.warning("Failed to parse Outlook datetime value: %s", value)
                    return value
            ...
            if outlook_event.get("isReminderOn"):
                reminder_minutes = outlook_event.get("reminderMinutesBeforeStart")
            ...
            if outlook_event.get("recurrence"):
                ...
                if pattern.get("type") == "daily":
                    recurrence_rule = (
                        f"FREQ=DAILY;INTERVAL={pattern.get('interval', 1)}"  # noqa: E501
                    )
                elif pattern.get("type") == "weekly":
                    recurrence_rule = (
                        f"FREQ=WEEKLY;INTERVAL={pattern.get('interval', 1)}"  # noqa: E501
                    )
    ...
    def _fetch_supported_timezone_mappings(self) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.warning("Failed to load Outlook supported time zones dynamically: %s", exc)
            return {}
    ...
    def _get_iana_to_windows_map(self) -> Dict[str, str]:
        if not self._timezone_map_loaded:
            ...
            if not mapping:
                ...
                self.logger.debug(
                    "Using default Outlook timezone mapping with %d entries", len(mapping)
                )
    ...
    def _to_windows_timezone(self, identifier: str, dt_value: datetime) -> str:
        ...
        self.logger.warning(
            "Unable to determine Outlook timezone for identifier '%s'; using UTC", identifier
        )
        return "UTC"

    def _convert_to_outlook_event(self, event: CalendarEvent) -> dict:
        """
        ...
        if event.description:
            outlook_event["body"] = {"contentType": "text", "content": event.description}
        ...
        if event.reminder_minutes is not None:
            ...
            outlook_event["reminderMinutesBeforeStart"] = event.reminder_minutes
    ...
    def _build_recurrence_payload(
        self, rule: str, start_dt: datetime, local_tz: Optional[tzinfo]
