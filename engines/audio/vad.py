...
class VADDetector:
    ...
    def __init__(self, threshold: float = 0.5, silence_duration_ms: int = 2000, 
                 method: str = 'silero'):
        """
        初始化 VAD 检测器
    ...
    def _detect_speech_silero(self, audio: np.ndarray, sample_rate: int) -> List[Dict]:
        ...
        try:
            ...
            for ts in speech_timestamps:
                ...
                start_sec = ts['start'] / sample_rate if isinstance(ts['start'], (int, float)) else ts['start']
                end_sec = ts['end'] / sample_rate if isinstance(ts['end'], (int, float)) else ts['end']
                result.append({
                    'start': start_sec,
                    'end': end_sec
                })
            ...
            logger.error(f"Silero VAD internal error: {e}. This may be a version compatibility issue.")
    ...
    def _detect_speech_webrtc(self, audio: np.ndarray, sample_rate: int) -> List[Dict]:
        ...
        try:
            ...
            if sample_rate not in [8000, 16000, 32000, 48000]:
                logger.warning(f"WebRTC VAD requires sample rate of 8k/16k/32k/48k, got {sample_rate}")
                return []
