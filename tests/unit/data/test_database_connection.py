    ...
    def test_get_cursor_rollback_on_error(self, tmp_path):
        ...
        with pytest.raises(sqlite3.IntegrityError):
            with db.get_cursor(commit=True) as cursor:
                ...
                cursor.execute("INSERT INTO test (id, value) VALUES (1, 'duplicate')")  # Duplicate key
...
class TestDatabaseSchemaOperations:
    ...
    def test_initialize_schema_custom_path(self, tmp_path):
        ...
        result = db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='custom_test'")
        assert len(result) == 1
    ...
    def test_get_version_with_value(self, tmp_path):
        ...
        db.execute("CREATE TABLE app_settings (key TEXT PRIMARY KEY, value TEXT, updated_at TEXT)", commit=True)
        db.execute("INSERT INTO app_settings VALUES ('schema_version', '5', '2024-01-01')", commit=True)
...
class TestDatabaseThreadSafety:
    ...
    def test_concurrent_writes(self, tmp_path):
        ...
        db.execute("CREATE TABLE test (id INTEGER PRIMARY KEY AUTOINCREMENT, value TEXT)", commit=True)
