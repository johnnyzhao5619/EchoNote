...
class TestTranscriptionManager:
    ...
    def manager(self, mock_db, mock_engine, temp_dir):
        ...
        workspace_manager.get_event_asset_path.return_value = None
        workspace_manager.read_event_text_asset.return_value = ""
        workspace_manager.has_event_asset_role.return_value = False
        workspace_manager.remove_event_asset_role.return_value = False
    ...
    @pytest.mark.asyncio
    async def test_completed_batch_task_publishes_workspace_item(self, manager, temp_audio_file, temp_dir):
        """Completed transcription tasks should publish artifacts to workspace."""
        task_id = manager.add_task(str(temp_audio_file))
        internal_path = temp_dir / f"{task_id}.json"

        with (
            patch.object(manager, "_get_internal_format_path", return_value=str(internal_path)),
            patch.object(manager, "export_result", return_value=str(temp_dir / "meeting.txt")),
            patch.object(manager, "_send_notification"),
        ):
            await manager._process_task_async(task_id, cancel_event=asyncio.Event())

        manager.workspace_manager.publish_transcription_task.assert_called_once()
    ...
    async def test_process_task_preserves_event_id_for_translation_save(
        ...
        manager.workspace_manager.publish_transcription_task.assert_called_once()
        assert manager.workspace_manager.publish_transcription_task.call_args.kwargs["event_id"] == "evt-1"
        assert manager.workspace_manager.publish_transcription_task.call_args.kwargs["translation_path"] is not None
    ...
    async def test_replace_realtime_retranslates_when_old_translation_exists(
        ...
        manager.workspace_manager.get_event_asset_path.side_effect = (
            lambda _event_id, asset_role: (
                str(transcript_path)
                if asset_role == "transcript"
                else str(translation_path) if asset_role == "translation" else None
            )
        )
        manager.workspace_manager.has_event_asset_role.side_effect = (
            lambda _event_id, asset_role: asset_role == "translation"
        )
        ...
        manager.translation_engine.translate.assert_awaited_once()
        manager.workspace_manager.publish_transcription_task.assert_called_once()
        assert manager.workspace_manager.publish_transcription_task.call_args.kwargs["replace_existing"] is True
    ...
    async def test_replace_realtime_rejects_degraded_transcript_output(
        ...
        manager.workspace_manager.get_event_asset_path.return_value = str(transcript_path)
        manager.workspace_manager.read_event_text_asset.return_value = existing_text
    ...
    async def test_replace_realtime_clears_stale_translation_attachment_when_refresh_fails(
        ...
        manager.workspace_manager.get_event_asset_path.side_effect = (
            lambda _event_id, asset_role: str(transcript_path) if asset_role == "transcript" else None
        )
