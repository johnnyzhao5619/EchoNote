...
Workspace 在产品定义上不是“搜索 + 查看”的只读资料页，而是统一承载手动笔记、导入资料、录音转写沉淀的文档工作台。左侧 SecondPanel 必须始终承担树形导航职责，右侧主区域必须承担文档创建、编辑、AI 加工、导出与导入后的继续整理。
...
/workspace          → WorkspacePanel + WorkspaceMain（默认态）
/workspace/:folderId → WorkspacePanel + WorkspaceMain（文件夹视图）
/workspace/:folderId/:docId → WorkspacePanel + DocumentMain（文件夹内文档）
/workspace/document/:documentId → WorkspacePanel + DocumentMain（无文件夹文档）
...
### 6.2.1 Workspace 信息架构

Workspace 必须围绕“树形组织 + 文档工作台”设计，而不是围绕“搜索结果 + 列表卡片”设计。

- 左侧 `WorkspacePanel`：递归文件夹树、当前路径高亮、展开/折叠、文件夹右键操作、新建文件夹、搜索入口
- 右侧 `WorkspaceMain`：基于路由展示当前文件夹的文档列表，承担“新建文档”和“导入文件”的主入口
- 文档详情 `DocumentMain`：承载标题编辑、正文编辑、录音转写资产、AI 资产、导出操作

树形导航的基本要求：

- 文件夹层级必须通过缩进、折叠箭头、展开状态显式表达
- 选择文件夹与展开/折叠不是同一语义，二者需要可区分的交互反馈
- 文档数量仅作为辅助信息，不能替代层级表达
- 顶层为空时也要明确呈现“新建文件夹 / 新建文档 / 导入文件”入口，避免空白界面

### 6.2.2 Workspace 核心交互闭环

1. 新建文档
   在当前文件夹下创建 `source_type='note'` 的文档，并立即进入文档详情页的可编辑正文态。新文档至少具备一个可编辑的 `document_text` 资产，不允许出现“文档已创建但正文不可写”的中间态。

2. 导入文档
   用户选择 `pdf/docx/txt/md` 文件后，系统解析文本、创建 `source_type='import'` 文档、写入 `document_text` 资产，并在导入成功后自动打开该文档进入可继续编辑状态。导入动作不是单纯的文件入库，而是“导入并接管为本地可编辑文档”。

3. 录音文档
   录音转写生成的文档仍属于同一工作台模型，允许在转写原文之外维护 `summary`、`meeting_brief`、`translation` 等资产；若产品允许人工修订，也应复用同一编辑基础设施，而不是额外做一套录音专用编辑器。

4. 搜索
   搜索用于跨树定位文档，而不是替代树形导航。搜索结果点击后必须落到对应文档路由，并保持左侧树与当前文档的归属关系一致。
...
useWorkspaceStore   // folders, documents, currentDoc, treeState, search, import/export, authoring
...
CREATE TABLE workspace_documents (
    ...
    file_path    TEXT,
    content_text TEXT,
    source_type  TEXT NOT NULL,            -- 'recording' | 'import' | 'note'
