        ...
        if retry_after:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse Retry-After header: " f"{retry_after}, error: {e}"
                    )
    ...
    def request(self, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # 成功，返回响应
                if attempt > 0:
                    logger.info(f"Request succeeded after {attempt} retries: " f"{method} {url}")
                ...
                if not self._is_retryable_error(e, response):
                    logger.error(
                        f"Non-retryable HTTP error: " f"{response.status_code} {method} {url}"
                    )
                    raise

                # 如果是最后一次尝试，抛出异常
                if attempt >= self.max_retries:
                    logger.error(f"Max retries exceeded: {method} {url}")
                    raise
                ...
                # 如果是最后一次尝试，抛出异常
                if attempt >= self.max_retries:
                    logger.error(f"Max retries exceeded for network error: {method} {url}")
                    raise
