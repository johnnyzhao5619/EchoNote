...
class MainWindow(QMainWindow):
    ...
    def _create_placeholder_pages(self, content_area: QStackedWidget):
        ...
        if (self.managers.get('transcription_manager') is not None):
            ...
            self._add_error_placeholder(content_area, 'batch_transcribe', 
        ...
        if self.managers.get('realtime_recorder') is not None:
            ...
            self._add_error_placeholder(content_area, 'realtime_record',
                                       'Realtime recorder not available')
        
        # Create calendar hub widget
        if (self.managers.get('calendar_manager') is not None and
            self.managers.get('oauth_manager') is not None):
            try:
        ...
        if self.managers.get('timeline_manager') is not None:
            ...
            self._add_error_placeholder(content_area, 'timeline',
                                       'Timeline manager not available')
        ...
        if self.managers.get('settings_manager') is not None:
            ...
            self._add_error_placeholder(content_area, 'settings',
                                       'Settings manager not available')
        
        logger.debug(f"Created {len(self.pages)} page widgets")
    
    def _add_error_placeholder(self, content_area: QStackedWidget, 
                               page_name: str, error_msg: str):
        """
    ...
    def apply_theme(self, theme: str):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Theme file not found: {theme_path}, using default"
                )
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        try:
            ...
            if self._has_running_tasks():
                ...
                if task_count > 0:
                    message = self.i18n.t(
                        'app.exit_confirmation_message_with_count',
                        count=task_count
                    )
                else:
    ...
    def _cleanup(self):
        ...
        try:
                ...
                try:
                    ...
                    if hasattr(transcription_manager, 'stop_all_tasks'):
                        ...
                        if transcription_manager._running:
                            logger.warning(
                                "Transcription manager did not stop within timeout"
                            )
                except Exception as e:
            ...
            if 'realtime_recorder' in self.managers:
                try:
                    realtime_recorder = self.managers['realtime_recorder']
                    if hasattr(realtime_recorder, 'is_recording') and realtime_recorder.is_recording:
            ...
            try:
                settings_manager = self.managers.get('settings_manager')
                if settings_manager:
                    settings_manager.api_keys_updated.disconnect(
                        self._on_api_keys_updated
                    )
            except Exception:
                pass
            ...
            if cleanup_duration > 10.0:
                logger.warning(
                    f"Cleanup took longer than expected: {cleanup_duration:.2f}s"
                )
        ...
        for page_name, page_widget in self.pages.items():
            if hasattr(page_widget, 'update_translations'):
                try:
                    ...
                    logger.error(
                        f"Error updating translations for page {page_name}: {e}"
                    )
