...
class TranscriptViewerDialog(QDialog):
    ...
    def _translate_file_error(self, error_msg: str) -> str:
        ...
        elif "invalid file encoding" in error_lower or "unicode" in error_lower or "decode" in error_lower:
    ...
    def _create_text_section(self) -> QFrame:
        ...
        self.text_edit.setContextMenuPolicy(
            self.text_edit.contextMenuPolicy()
        )
    ...
    def save_changes(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                self.i18n.t('viewer.save_success')
            )
            ...
            self._show_save_error_with_retry(
                self.i18n.t('viewer.save_error_permission'),
                str(e)
            )
            ...
            self._show_save_error_with_retry(
                self.i18n.t('viewer.save_error_details', error=str(e)),
                str(e)
            )
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                segments.append({
                    'start': start_time,
                    'end': end_time,
                    'text': text.strip() if text else line
                })
    ...
    def _show_export_error(self, format: str, error_msg: str):
        ...
        # Add buttons
        retry_button = msg_box.addButton(
            self.i18n.t('viewer.export_retry'),
            QMessageBox.ButtonRole.AcceptRole
        )
        msg_box.addButton(
            self.i18n.t('common.cancel'),
    ...
    def _text_context_menu_event(self, event):
        ...
        select_all_action = QAction(
            self.i18n.t('viewer.select_all'), self
        )
    ...
    def update_language(self):
        ...
        # Window title
        self.setWindowTitle(
            self.i18n.t('viewer.title', filename=self.task_data['file_name'])
        )
        ...
        self.engine_label.setText(
            self.i18n.t('viewer.metadata.engine') + f": {self.engine_value}"
        )
