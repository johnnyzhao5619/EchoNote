    ...
    def _check_availability(self):
        ...
        if self.system == "Windows":
            try:
                ...
                logger.warning(
                    "win10toast not installed, Windows notifications unavailable"
                )
            ...
            try:
                ...
                if not self.linux_available:
                    logger.warning(
                        "notify-send not found, Linux notifications unavailable"
                    )
    ...
    def send_notification(
        ...
        try:
            if self.system == "Darwin":  # macOS
                ...
                self._send_linux_notification(
                    title, message, notification_type, duration
                )
            else:
                logger.warning(
                    f"Notifications not supported on {self.system}"
                )
