...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QCheckBox, QLabel, QMessageBox, QGroupBox
)
...
class EventDialog(QDialog):
    ...
    def __init__(
        ...
        logger.debug(
            f"EventDialog initialized in "
            f"{'edit' if self.is_edit_mode else 'create'} mode"
        )
    ...
    def setup_ui(self):
        ...
        title = (
            "Edit Event" if self.is_edit_mode else
    ...
    def _create_sync_options(self) -> QGroupBox:
        ...
        for provider, email in self.connected_accounts.items():
            checkbox = QCheckBox(
                f"Sync to {provider.capitalize()} ({email})"
            )
    ...
    def _create_buttons(self) -> QHBoxLayout:
        ...
        # Save button
        save_text = (
            self.i18n.t('common.save') if self.is_edit_mode else
            self.i18n.t('calendar_hub.create_event')
        )
    ...
    def _populate_form(self):
        ...
        if 'description' in self.event_data:
            self.description_input.setPlainText(
                self.event_data['description'] or ''
            )
