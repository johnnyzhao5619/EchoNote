    ...
    def _create_settings_pages(self):
        ...
        try:
            ...
            pages = [
                ...
                ('realtime', RealtimeSettingsPage(
                    self.settings_manager, self.i18n
                )),
                ('calendar', CalendarSettingsPage(
                    self.settings_manager, self.i18n, self.managers
                )),
                ('timeline', TimelineSettingsPage(
                    self.settings_manager, self.i18n
                )),
    ...
    def save_settings(self) -> bool:
        ...
        try:
            ...
            for page_id, page_widget in self.settings_pages.items():
                if hasattr(page_widget, 'validate_settings'):
                    ...
                    if not is_valid:
                        QMessageBox.warning(
                            self,
                            self.i18n.t('settings.validation.title'),
                            error_msg
                        )
            ...
            QMessageBox.information(
                ...
                self.i18n.t('settings.success.saved')
            )
