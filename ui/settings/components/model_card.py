...
class ModelCardWidget(QFrame):
    ...
    def _update_state(self):
        ...
        if self.model.is_downloaded:
            ...
            if last_used:
                ...
                time_label = QLabel(
                    f"{self.i18n.t('settings.model_management.last_used')}: {last_used_text}"
                )
            else:
    ...
    def update_progress(self, progress: float):
        ...
        for i in range(self.actions_layout.count()):
            ...
            if isinstance(widget, QPushButton) and widget.text() == self.i18n.t(
                "settings.model_management.download"
            ):
