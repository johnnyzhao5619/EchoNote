...
class DataManagementPage(BasePage):
    ...
    def _refresh_storage_summary(self):
        ...
        try:
            ...
            self.config_label.setText(
                f"Configuration: {config_size} "
                f"({len(summary['config_files'])} files)"
            )
            self.recordings_label.setText(
                f"Recordings: {recordings_size} "
                f"({summary['recordings_count']} files)"
            )
            self.transcripts_label.setText(
                f"Transcripts: {transcripts_size} "
                f"({summary['transcripts_count']} files)"
            )
            self.logs_label.setText(
                f"Logs: {logs_size} "
                f"({summary['logs_count']} files)"
            )
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to calculate storage summary:\n{str(e)}"
            )
    ...
    def _execute_cleanup(self):
        ...
        self.cleanup_worker.error.connect(
            lambda error: self._on_cleanup_error(error, progress)
        )
    ...
    def _on_cleanup_finished(self, results: dict, progress: QProgressDialog):
        ...
        if results['success']:
            ...
            msg_box.setText(
                "Data cleanup completed, but some items could not be deleted."
            )
            ...
            logger.warning(
                f"Data cleanup completed with {len(results['failed_items'])} errors"
            )
