...
class TranscriptionSettingsPage(BaseSettingsPage):
    ...
    def setup_ui(self):
        ...
        self.concurrent_label = QLabel(
            self.i18n.t('settings.transcription.concurrent_tasks')
        )
        ...
        # FFmpeg info
        self.ffmpeg_info = QLabel(
            self.i18n.t('settings.transcription.ffmpeg_info')
        )
        ...
        self.browse_button = QPushButton(
            self.i18n.t('settings.transcription.browse')
        )
        ...
        self.engine_label = QLabel(
            self.i18n.t('settings.transcription.engine_select')
        )
    ...
    def _create_engine_configs(self):
        ...
        self.whisper_group = QGroupBox(
            self.i18n.t('settings.transcription.whisper_config')
        )
        ...
        whisper_layout.addRow(
            self.compute_type_label,
            self.compute_type_combo
        )
        ...
        self.cloud_group = QGroupBox(
            self.i18n.t('settings.transcription.cloud_config')
        )
        cloud_layout = QVBoxLayout()
        
        # Note about API keys
        self.note_label = QLabel(
            self.i18n.t('settings.transcription.api_key_note')
        )
    ...
    def _update_model_list(self):
        ...
        if not downloaded_models:
            ...
            logger.info(
                f"Updated model list with {len(downloaded_models)} models"
            )
    ...
    def _create_api_key_section(self, parent_layout: QVBoxLayout):
        ...
        self.openai_group = QGroupBox(
            self.i18n.t('settings.transcription.cloud_api_openai')
        )
        ...
        openai_layout.addRow(
            self.openai_api_key_label,
            key_layout
        )
        
        # Usage statistics
        self.openai_usage_label = QLabel(
            self.i18n.t('settings.transcription.no_usage_data')
        )
        self.openai_usage_label.setStyleSheet("color: #666;")
        self.openai_monthly_usage_label = QLabel(self.i18n.t('settings.transcription.monthly_usage'))
        openai_layout.addRow(
            self.openai_monthly_usage_label,
            self.openai_usage_label
        )
        ...
        self.google_group = QGroupBox(
            self.i18n.t('settings.transcription.cloud_api_google')
        )
        ...
        self.google_show_button.clicked.connect(
            lambda: self._toggle_password_visibility(
                ...
                self.google_show_button
            )
        )
        google_key_layout.addWidget(self.google_show_button)
        
        self.google_test_button = QPushButton(
            self.i18n.t('settings.transcription.test_connection')
        )
        self.google_test_button.clicked.connect(
            lambda: self._test_api_key('google')
        )
        ...
        google_layout.addRow(
            self.google_api_key_label,
            google_key_layout
        )
        
        self.google_usage_label = QLabel(
            self.i18n.t('settings.transcription.no_usage_data')
        )
        self.google_usage_label.setStyleSheet("color: #666;")
        self.google_monthly_usage_label = QLabel(self.i18n.t('settings.transcription.monthly_usage'))
        google_layout.addRow(
            self.google_monthly_usage_label,
            self.google_usage_label
        )
        ...
        # Azure API Key
        self.azure_group = QGroupBox(
            self.i18n.t('settings.transcription.cloud_api_azure')
        )
        ...
        self.azure_show_button.clicked.connect(
            lambda: self._toggle_password_visibility(
                self.azure_key_edit,
                self.azure_show_button
            )
        )
        azure_key_layout.addWidget(self.azure_show_button)
        
        self.azure_test_button = QPushButton(
            self.i18n.t('settings.transcription.test_connection')
        )
        self.azure_test_button.clicked.connect(
            lambda: self._test_api_key('azure')
        )
        ...
        azure_layout.addRow(
            self.azure_monthly_usage_label,
            self.azure_usage_label
        )
        ...
        try:
            ...
            for provider, label in [
                ...
                if usage and usage.get('usage_count', 0) > 0:
                    ...
                    # Format usage text
                    usage_text = (
                        f"{count} calls, {duration_minutes:.1f} min, "
                        f"${cost:.2f}"
                    )
    ...
    def _update_device_info(self):
        ...
        if device_id == 'auto':
            ...
            self.device_info_label.setText(
                "NVIDIA GPU acceleration. Requires CUDA-compatible GPU."
            )
        elif device_id == 'cpu':
    ...
    def _on_browse_clicked(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self,
            self.i18n.t('settings.transcription.select_directory'),
            current_path
        )
    ...
    def load_settings(self):
        ...
        try:
            # General settings
            output_format = self.settings_manager.get_setting(
                'transcription.default_output_format'
            )
            if output_format:
            ...
            save_path = self.settings_manager.get_setting(
                'transcription.default_save_path'
            )
            if save_path:
                # Expand user path
            ...
            # Engine settings
            engine = self.settings_manager.get_setting(
            ...
            whisper_settings = self.settings_manager.get_setting(
    ...
    def save_settings(self):
        ...
        try:
            ...
            if 'transcription_manager' in self.managers:
                try:
                    self.managers['transcription_manager'].update_max_concurrent(
                        new_max_concurrent
                    )
                    logger.info(
                        f"Updated transcription manager max_concurrent to "
                        f"{new_max_concurrent}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to update transcription manager "
                        f"max_concurrent: {e}"
                    )
            ...
            self.settings_manager.set_setting(
                ...
                device_id if device_id else 'auto'
            )
            
            self.settings_manager.set_setting(
                'transcription.faster_whisper.compute_type',
                self.compute_type_combo.currentText()
            )
    ...
    def validate_settings(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, self.i18n.t(
                'settings.transcription.error.invalid_path',
                error=str(e)
            )
    ...
    def update_translations(self):
        ...
        if hasattr(self, 'ffmpeg_status_text'):
            ...
            if ffmpeg_checker.is_ffmpeg_available() and ffmpeg_checker.is_ffprobe_available():
                ...
                self.ffmpeg_status_text.setText(self.i18n.t('settings.transcription.ffmpeg_not_installed'))
        ...
        if hasattr(self, 'azure_region_label'):
            self.azure_region_label.setText(self.i18n.t('settings.transcription.azure_region_label'))
        
        # Update monthly usage labels
        if hasattr(self, 'openai_monthly_usage_label'):
            self.openai_monthly_usage_label.setText(self.i18n.t('settings.transcription.monthly_usage'))
        if hasattr(self, 'google_monthly_usage_label'):
            self.google_monthly_usage_label.setText(self.i18n.t('settings.transcription.monthly_usage'))
        if hasattr(self, 'azure_monthly_usage_label'):
            self.azure_monthly_usage_label.setText(self.i18n.t('settings.transcription.monthly_usage'))
        ...
        if hasattr(self, 'openai_key_edit'):
            self.openai_key_edit.setPlaceholderText(self.i18n.t('settings.transcription.api_key_placeholder'))
        if hasattr(self, 'azure_region_edit'):
            self.azure_region_edit.setPlaceholderText(self.i18n.t('settings.transcription.azure_region_placeholder'))
