...
fn decode_wav_to_pcm(path: &Path) -> Result<Vec<f32>, String> {
    ...
    crate::audio::normalize_audio_for_whisper(
        &interleaved,
        spec.sample_rate,
...
mod tests {
    ...
    #[test]
    fn test_decode_wav_to_pcm_normalizes_to_whisper_sample_rate_and_mono() {
        let temp = tempfile::NamedTempFile::new().expect("temp wav");
        ...
        {
            let mut writer = hound::WavWriter::create(temp.path(), spec).expect("create wav");
            for frame in 0..44_100 {
                let sample = (2.0 * PI * 440.0 * frame as f32 / 44_100.0).sin();
                let pcm = (sample * i16::MAX as f32) as i16;
                writer.write_sample(pcm).expect("left");
                writer.write_sample(pcm).expect("right");
            }
            writer.finalize().expect("finalize wav");
        }

        let pcm = decode_wav_to_pcm(temp.path()).expect("decode wav");

        assert!(
            pcm.len() >= 15_500 && pcm.len() <= 16_500,
            "expected ~16000 mono samples for whisper, got {}",
            pcm.len()
        );
    }
}
