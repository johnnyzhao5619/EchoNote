...
- Released `v1.4.2` with realtime audio input dynamic refresh support (manual refresh + idle auto refresh) so newly attached microphones can be discovered without restarting.
- Refined realtime floating window resizing behavior: header/meta/actions keep fixed spacing while extra height is allocated to transcript/translation preview areas for better readability.
- Synchronized project, documentation, and landing metadata version references to `v1.4.2` for release consistency.
...
- Released `v1.3.6` with timeline, calendar hub, and auto start/stop logic fixes and optimizations to improve user experience.
- Updated canonical project and landing version references to `v1.3.6`.
- Updated landing package metadata to `1.3.6` for release consistency.
