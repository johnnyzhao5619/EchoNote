...
class MigrationManager:
    ...
    def get_pending_migrations(self) -> List[Tuple[int, Path]]:
        ...
        pending = [(version, path) for version, path in all_migrations if version > current_version]

        logger.info(
            f"Current schema version: {current_version}, " f"Pending migrations: {len(pending)}"
        )
